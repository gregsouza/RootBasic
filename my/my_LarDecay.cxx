...
public:
  ...
  my_LarDecay(const my_LarDecay& other, const char* name):
    ...
    _fast("fast", this, other._fast),
    _slow("slow", this, other._slow),
...
void myLarDecay(){
  //time
  RooRealVar t("t", "t", 0., 20000.);

  //gaussian model
  RooRealVar mean("mean", "mean", 500.);
  RooRealVar sigma("sigma", "sigma", 50.);
  RooGaussModel gaussm("gauss", "gauss", t, mean, sigma);
  
  //decay distribution
  RooRealVar tau1("tau", "tau", 9.);
  RooRealVar tau2("tau2", "tau2", 1500.);
  RooRealVar A1("A1", "A1", 0.33);
  RooRealVar A2("A2", "A2", 0.77);
